"""프로세스 내 in-flight 프로젝트 추적 (Gemini POC 전용, OpenCLIP의 app/state.py와 완전히 분리된 세트).

DB의 gemini_analysis_runs.status='processing' 이 영속적 가드 역할을 하고,
이 메모리 set은 같은 프로세스 내에서의 빠른 중복 요청을 즉시 막기 위한 보조 가드다.
OpenCLIP 분석(app/state.py)과 별개의 가드이므로 두 분석은 서로를 막지 않는다.
"""
import threading
